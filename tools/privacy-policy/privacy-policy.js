/**
 * 隐私政策生成与发布（子页面 /tools/privacy-policy/）
 * 与根目录 script.js 同源逻辑；正式链接须指向站点根目录下的 agreement.html。
 */
(function () {
  ...
  /** 站点根路径前缀（支持部署在子目录，如 /repo/tools/privacy-policy/） */
  function getSiteRootPrefix() {
    var path = window.location.pathname || "/";
    var key = "/tools/privacy-policy";
  ...
  function buildPublishUrlByCode(shortCode) {
    var rootBase = window.location.origin + getSiteRootPrefix();
    if (!rootBase.endsWith("/")) rootBase += "/";
    var url = new URL("agreement.html", rootBase);
